...
public class StaticDataEditor : EditorWindow
{
    ...
    // Staging timeline
    private readonly Dictionary<int, StagedEventCollection> _stagedCollections =
        new Dictionary<int, StagedEventCollection>();
    ...
    [MenuItem("SDK/Game Tools/Static Data Editor")]
    ...
    private void OnGUI()
    {
        ...
        {
            staticData = (AnimatorControllerStaticData)
                EditorGUILayout.ObjectField(
                    ...
                    staticData,
                    typeof(AnimatorControllerStaticData),
                    false
                );
            using (new EditorGUI.DisabledScope(staticData == null))
            {
                if (
                    GUILayout.Button(
                        new GUIContent(
                            "Fill Event",
                            "Grab the event from the current selected index" + " of the static data"
                        ),
                        GUILayout.Width(80)
                    )
                )
        ...
        eventsCollectionIndex = Mathf.Max(
            0,
            EditorGUILayout.IntField(
                new GUIContent(
                    ...
                    "Select the index you want to modify."
                        ...
                        + " always +1 of your highest element number."
                ),
        ...
        if (staticData == null)
        {
            EditorGUILayout.HelpBox(
                "Please assign an AnimatorControllerStaticData object.",
                MessageType.Warning
            );
        }
        else
    ...
    private void DrawSoundContainerSection()
    {
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            soundContainer = (GameObject)
                EditorGUILayout.ObjectField(
                    new GUIContent(
                        "Container",
                        "Insert the weapon container prefab" + " that stores the WeaponSoundPlayer"
                    ),
                    soundContainer,
                    typeof(GameObject),
                    false
                    ...
                    GUILayout.Button(
                        new GUIContent(
                            ...
                            "Refresh both the sound library"
                                + " and animation prefab that is inserted"
                        ),
                        GUILayout.Width(80)
                    )
                )
        ...
        {
            EditorGUILayout.HelpBox(
                "Assign a prefab with a WeaponSoundPlayer component to enable Sound event playback.",
                MessageType.Info
            );
        }
        ...
        {
            EditorGUILayout.HelpBox(
                $"Loaded {_soundLibrary.Count} sound event(s) from WeaponSoundPlayer.",
                MessageType.None
            );
        }
    }
    ...
    private void RefreshSoundLibrary()
    {
        ...
        if (
            !_soundAccessor.TryGetSoundEvents(
                soundContainer,
                out List<SoundEventEntry> entries,
                out string error
            )
        )
        {
    ...
    private void DrawStaticDataEditor()
    {
        ...
        {
            EditorGUILayout.HelpBox(
                "Could not read event collections from this asset (reflection target missing).",
                MessageType.Error
            );
            return;
        }
        ...
        EditorGUILayout.HelpBox(
            $"Editing Event Collection Index {eventsCollectionIndex}  \u2022  {staged.Events.Count} event(s) staged",
            MessageType.None
        );
    ...
    private void DrawTimeline(StagedEventCollection staged)
    {
            ...
            GUILayout.Label(
                new GUIContent(
                    ...
                    "Timeline for displaying the Animation Event that exist within the current static data index."
                        ...
                        + " where the event will be placed."
                ),
                EditorStyles.boldLabel
            );
            GUILayout.FlexibleSpace();
            GUILayout.Label("Zoom", GUILayout.Width(36));
            _timelineZoom = GUILayout.HorizontalSlider(
                _timelineZoom,
                1f,
                10f,
                GUILayout.Width(120)
        ...
        hideSoundEvents = EditorGUILayout.Toggle(
            new GUIContent("Hide Sound Event", "Hide the Event that is only playing sound"),
            hideSoundEvents,
            GUILayout.Width(60)
        );
        hideEventAmount = Mathf.Max(
            0,
            EditorGUILayout.IntField(
                new GUIContent(
                    "Hide Event Amount",
        ...
        _timelineScrollPos = GUI.BeginScrollView(
            viewportRect,
            _timelineScrollPos,
            contentRect,
            true,
        ...
        if (animationClip != null && animationClip.length > 0f)
        {
            float playheadX =
                trackRect.x
                + trackRect.width * Mathf.Clamp01(_preview.AnimationTime / animationClip.length);
            ...
            playheadX =
                ...
                + trackRect.width * Mathf.Clamp01(_preview.AnimationTime / animationClip.length);
            EditorGUI.DrawRect(
                new Rect(playheadX - 1, trackRect.y, 2, trackRect.height),
                Color.red
            );
        }
    ...
    private void ApplyTimelineZoom(Rect viewportRect, Event e)
    {
        ...
        if (!Mathf.Approximately(newZoom, oldZoom))
        {
            ...
            _timelineScrollPos.x =
                fraction * newContentWidth - (e.mousePosition.x - viewportRect.x);
        }
    ...
    private void DrawTimelineRuler(Rect trackRect)
    {
        ...
        for (int i = 0; i <= 10; i++)
        {
            ...
            EditorGUI.DrawRect(
                new Rect(x, trackRect.y, 1, trackRect.height),
    ...
    private void DrawTimelineBoxes(StagedEventCollection staged, Rect trackRect)
    {
        ...
        var centeredNumberStyle = new GUIStyle(EditorStyles.whiteBoldLabel)
        {
        ...
        var boxes = new List<(Rect rect, Color color, bool expanded, string caption)>(
            staged.Events.Count
        );
        ...
        for (int i = 0; i < staged.Events.Count; i++)
        {
            ...
            EditorGUI.DrawRect(
                new Rect(anchorX - 1, trackRect.y, 2, trackRect.height),
        ...
        for (int pass = 0; pass < 2; pass++)
        {
            ...
            foreach (var box in boxes)
            {
                ...
                GUI.Label(
                    box.rect,
                    box.caption,
                    box.expanded ? EditorStyles.whiteMiniLabel : centeredNumberStyle
                );
            }
        }
        ...
        if (_draggingStagedIndex >= 0 && _draggingStagedIndex < staged.Events.Count)
        {
            if (e.type == EventType.MouseDrag)
            {
                ...
                dragged.NormalizedTime = Mathf.Clamp01(
                    (e.mousePosition.x - trackRect.x) / trackRect.width
                );
    ...
    private void DrawEventInspector(StagedEventCollection staged)
    {
        ...
        GUILayout.Label(
            hasSelection
                ? $"Editing Event #{staged.Events[_selectedStagedIndex].CreationOrder}"
                : "New Event",
            EditorStyles.boldLabel
        );
        ...
        DrawSearchableDropdownField(
            ...
            true,
            "List of Function names that is triggered during the event by the client"
        );

        string functionName = AnimationEventDefinitions.FunctionNames[selectedFunctionIndex];
        bool hasParameter = AnimationEventDefinitions.FunctionsWithParameters.Contains(
            functionName
        );
        ...
        showEventConditions = EditorGUILayout.Toggle(
            new GUIContent(
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            {
                StagedAnimationEvent newEvent = BuildStagedEventFromFields(
                    staged.NextCreationOrder++
                );
                newEvent.NormalizedTime =
            ...
            using (new EditorGUI.DisabledScope(!hasSelection))
            {
                if (
                    GUILayout.Button(
                        new GUIContent(
                            "Update Event",
                            "Update the selected Animation Event" + " from the timeline"
                        )
                    )
                )
                {
    ...
    private void DrawAnimationEventParameterFields(string functionName)
    {
        ...
        {
            case "Sound":
                useContainerSound = EditorGUILayout.Toggle(
                    new GUIContent(
                        "Use Container Sound",
                        "Instead of manually typing the Event name,"
                            ...
                            + " Requires Weapon Container prefab to be inserted"
                    ),
                    useContainerSound
                );
                ...
                paramType = EditorGUILayout.Popup(
                    "Param Type",
                    paramType,
                    AnimationEventDefinitions.ParamTypeNames
                );
                paramType = 3;
                break;
            case "ThirdAction":
    ...
    private void DrawContainerSoundDropdown()
    {
        ...
        {
            EditorGUILayout.HelpBox(
                "No sounds loaded from the Container. Assign a Container with a WeaponSoundPlayer above.",
                MessageType.Warning
            );
            return;
        }
        ...
        DrawSearchableDropdownField(
            ...
            true,
            "Automatically grabs Event Name from the Weapon Container AdditionalSounds and remove the \"Snd\" prefix"
    ...
    /// </summary>
    private static void DrawSearchableDropdownField(
        string label,
        string[] options,
        int currentIndex,
        ...
        if (isTooltip)
        {
            if (
                EditorGUI.DropdownButton(
                    fieldRect,
                    new GUIContent(currentLabel, description),
                    FocusType.Keyboard
                )
            )
            {
    ...
    {
        condType = EditorGUILayout.Popup(
            "Condition Type",
            condType,
            AnimationEventDefinitions.ConditionTypeNames
        );
        ...
        DrawSearchableDropdownField(
            ...
            true,
            "Names for the conditions, will be different depending on Condition Type chosen."
        ...
        switch (condType)
        {
            case 0: // Int
                ...
                mode = EditorGUILayout.Popup(
                    "Condition Mode",
                    mode,
                    AnimationEventDefinitions.ConditionModeNames
                );
                break;
            case 1: // Float
    ...
    private void ApplyFieldsToStaged(StagedAnimationEvent s)
    {
        ...
        if (showEventConditions)
        {
            ...
            string parameterName =
                conditionNameEnum < names.Length ? names[conditionNameEnum] : string.Empty;
            ...
            switch (condType)
            {
                case 0:
                    ...
                    break;
                case 1:
                    c.FloatValue = condFloat;
                    c.IntValue = 0;
                    c.BoolValue = false;
                    c.ModeIndex = mode;
                    break;
                case 2:
                    c.BoolValue = condBool;
                    c.FloatValue = 0f;
                    c.IntValue = 0;
                    c.ModeIndex = 0;
                    break;
    ...
    {
        selectedFunctionIndex = Mathf.Max(
            0,
            Array.IndexOf(AnimationEventDefinitions.FunctionNames, s.FunctionName)
        );
    ...
    {
        if (
            !_stagedCollections.TryGetValue(eventsCollectionIndex, out StagedEventCollection staged)
        )
        {
    ...
    private void ReloadStagedFromStaticData()
    {
        ...
        _stagedCollections[eventsCollectionIndex] = StagedEventCollection.LoadFrom(
            eventsCollection,
            _accessor,
            animationClip
        );
    ...
    {
        var collection = new EventsCollection(new List<AnimationEventSystem.AnimationEvent>());
        _accessor.SetAnimationEvents(collection, collection._animationEvents);
        return collection;
    }
    ...
    private void DrawAnimationPreviewSection()
    {
        ...
        {
            AnimationClip newClip = (AnimationClip)
                EditorGUILayout.ObjectField(
                    new GUIContent(
                        "Animation Clip",
        ...
        userPreviewObject = (GameObject)
            EditorGUILayout.ObjectField(
                new GUIContent("User Preview Prefab", "Prefab for previewing the animation"),
                userPreviewObject,
                typeof(GameObject),
                false
            );
        ...
        {
            EditorGUILayout.HelpBox(
                "Please assign an Animation Clip or an Animation Prefab.",
                MessageType.Warning
            );
            return;
        }
        ...
        float newProgress = EditorGUILayout.Slider(
            "Progress",
            _preview.AnimationTime / animationClip.length,
            0f,
            1f
        ...
        if (_preview.IsPlaying)
        {
            ...
            if (
                staticData != null
                && _stagedCollections.TryGetValue(
                    eventsCollectionIndex,
                    out StagedEventCollection staged
                )
            )
            {
                _soundPlayer.CheckAndPlay(
                    beforeNormalized,
                    afterNormalized,
                    staged.Events,
                    _soundLibrary
