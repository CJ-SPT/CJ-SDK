...
namespace DebugPlus.Patches
{
    ...
    internal class LogErrorPatch : ModulePatch
    {
        ...
        {
            if (!DebugPlusConfig.UnityErrorLogging.Value)
                return;

            if (message.GetType() == typeof(string))
            {
                DebugPlus.Log.LogError(Format.FormatString((string)message));
                return;
            }

            DebugPlus.Log.LogError($"OBJECT: {message}");
        }
    }
    ...
    internal class LogErrorFormatPatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            return AccessTools
                .GetDeclaredMethods(typeof(Debug))
                .SingleDebug(m =>
                    m.Name == nameof(Debug.LogErrorFormat)
        ...
        public static void PatchPostfix(string format, object[] args)
        {
            ...
            DebugPlus.Log.LogError(Format.FormatString(format, args));
        }
    }
    ...
    internal class LogErrorFormatObjPatch : ModulePatch
    {
        ...
        public static void PatchPostfix(Object context, string format, object[] args)
        {
            ...
            DebugPlus.Log.LogError($"GameObject {context}\n{Format.FormatString(format, args)}");
        }
    }
}
