...
{
    private List<BotZone> _botZones = new();
    ...
    public void RefreshZones(object obj, EventArgs e)
    {
        ...
        foreach (var point in _spawnPointInfos)
        {
            var provider = point.Sphere.GetOrAddComponent<OverlayProvider>();
            provider.SetOverlayContent(point.Content, Enable, MaxDist, UpOffset, FontSize);
        }
    }
    ...
    private void CreateSpawnPointInfo(ISpawnPoint spawnPoint, BotZone zone, Color color)
    {
        ...
        var pointInfo = new SpawnPointInfo() { Sphere = sphere, Content = infoText };
    ...
    private static string GetPointInfoText(ISpawnPoint spawnPoint, BotZone zone)
    {
        ...
        OCB.AppendLabeledValue(
            "IsSniper",
            spawnPoint.IsSnipeZone.ToString(),
            Color.gray,
            Color.green
        );
